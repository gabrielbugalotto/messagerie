    ...
    .factory("mailService", () => {
        ...
        return {
            ...
            getDossier: function (valDossier) {
                return _.find(dossiers, (elt) => { return elt.value == valDossier });
                return null;
            },
            getMail: function (valDossier, idMail) {
                var dossierSelectionne = this.getDossier(valDossier);
                if (dossierSelectionne) {
                    return _.find(dossierSelectionne.emails, (elt) => { return elt.id == idMail });
                }
                return null;
            },
            envoiMail: function (mail) {
                ...
                mail.id = 0;
                _.forEach(dossiers, (elt) => { mail.id += elt.emails.length });
                mail.id++;
